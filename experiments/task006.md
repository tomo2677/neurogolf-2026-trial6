...
- Revisited after task010/task002 `ReduceSum` height/count wins: task006 has no dynamic height rank or valid-area scalar. Its two 3x3 slices are fixed, and replacing the overlap `Min(left_u8,right_u8)` with reductions would add tensors rather than remove them.
