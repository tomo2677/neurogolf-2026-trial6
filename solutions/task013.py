...
def _color_at_coord(nodes: list[onnx.NodeProto], row: str, col: str, output: str) -> None:
    nodes.extend(
        [
            helper.make_node("Reshape", [row, "one_i64"], [f"{output}_row1"]),
            helper.make_node("Reshape", [col, "one_i64"], [f"{output}_col1"]),
            helper.make_node("Add", [row, "one_i64"], [f"{output}_row_end"]),
            helper.make_node("Add", [col, "one_i64"], [f"{output}_col_end"]),
            helper.make_node("Reshape", [f"{output}_row_end", "one_i64"], [f"{output}_row_end1"]),
            helper.make_node("Reshape", [f"{output}_col_end", "one_i64"], [f"{output}_col_end1"]),
