...
def build_model() -> onnx.ModelProto:
    ...
    initializers = [
        ...
        _int64_tensor("green_ends", [1, 4, 30, 30]),
        _f32_tensor("row_idx30", [float(v) for v in range(30)], [1, 1, 30, 1]),
    ...
    nodes = [
        ...
        helper.make_node("Less", ["row_idx30", "row_count"], ["row_valid"]),
        ...
        helper.make_node("MaxPool", ["green_mask_u8"], ["left_seen"], kernel_shape=[1, 30], pads=[0, 29, 0, 0]),
        helper.make_node("MaxPool", ["green_mask_u8"], ["right_seen"], kernel_shape=[1, 30], pads=[0, 0, 0, 29]),
        helper.make_node("MaxPool", ["green_mask_u8"], ["up_seen"], kernel_shape=[30, 1], pads=[29, 0, 0, 0]),
        helper.make_node("MaxPool", ["green_mask_u8"], ["down_seen"], kernel_shape=[30, 1], pads=[0, 0, 29, 0]),
