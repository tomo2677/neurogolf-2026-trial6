...
def _horizontal_closure(nodes: list[onnx.NodeProto], seed: str, prefix: str, final: bool = False) -> str:
    nodes.extend(
        [
            ...
            helper.make_node("Gather", [f"{prefix}_seed_f32", "reverse_idx"], [f"{prefix}_rev"], axis=3),
            ...
            helper.make_node(
                "Greater",
                [f"{prefix}_rev_cum_f16", f"{prefix}_rev_last_green_cum"],
                [f"{prefix}_right_rev"],
            ),
...
def _vertical_closure(nodes: list[onnx.NodeProto], seed: str, prefix: str, final: bool = False) -> str:
    nodes.extend(
        [
            ...
            helper.make_node("Gather", [f"{prefix}_seed_f32", "reverse_idx"], [f"{prefix}_rev"], axis=2),
...
def build_model() -> onnx.ModelProto:
    ...
    nodes = [
        ...
        helper.make_node("Cast", ["seed_bool"], ["external_0"], to=onnx.TensorProto.FLOAT16),
    ]
