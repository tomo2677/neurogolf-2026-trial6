...
def _shift_row(nodes: list[onnx.NodeProto], source: str, output: str) -> None:
    nodes.extend(
        [
            helper.make_node("Slice", [source, "row_prev_starts", "row_prev_ends"], ["row_prev_crop"]),
...
def _shift_col(nodes: list[onnx.NodeProto], source: str, output: str) -> None:
    nodes.extend(
        [
            helper.make_node("Slice", [source, "col_prev_starts", "col_prev_ends"], ["col_prev_crop"]),
...
def build_model() -> onnx.ModelProto:
    ...
    initializers = [
        ...
        _int64_tensor("row_prev_starts", [0, 0, 0, 0], [4]),
    ...
    graph = helper.make_graph(nodes, "task021_separator_block_count_graph", [x], [y], initializers)
