...
def build_model() -> onnx.ModelProto:
    ...
    nodes.extend(
        ...
        helper.make_node(
            "Concat",
            ["false_col1"] + _gap3() + _gap3() + ["fill01"] + _gap3() + ["fill02", "false_col1"],
            ["fill_row0"],
            axis=3,
        ),
        helper.make_node(
            "Concat",
            ["false_col1", "fill10"] + _gap3() + ["fill11"] + _gap3() + ["fill12", "false_col1"],
            ["fill_row1"],
            axis=3,
        ),
        helper.make_node(
            "Concat",
            ["false_col1", "fill20"] + _gap3() + ["fill21"] + _gap3() + ["fill22", "false_col1"],
            ["fill_row2"],
            axis=3,
        ),
        helper.make_node(
            "Concat",
            ["false_row1", "fill_row0"] + _rows3() + ["fill_row1"] + _rows3() + ["fill_row2", "false_row1"],
            ["fill17"],
            axis=2,
        ),
    ...
    graph = helper.make_graph(nodes, "task033_slice_axes_graph", [x], [y], initializers)
