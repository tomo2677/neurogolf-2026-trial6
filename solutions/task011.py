...
def build_model() -> onnx.ModelProto:
    ...
    initializers = [
        ...
        _int64_tensor("unsq_row_axis", [1]),
        _int64_tensor("unsq_col_axis", [0]),
        _int64_tensor("unsq_last_axis", [2]),
        _int64_tensor("unsq_batch_axes", [0, 1]),
        _int64_tensor("shape33", [3, 3]),
        _int64_tensor("gathernd_index_shape", [1, 10, 3, 3, 2]),
    ...
    for i, start in enumerate(STARTS):
        initializers.append(_int64_tensor(f"idx{i}", [start, start + 1, start + 2], [3]))
        ...
        for bc, col in enumerate(STARTS):
            ...
            nodes.extend(
                [
                    ...
                    helper.make_node("Where", [f"{name}_no8", f"idx{br}", row_idx], [f"row_idx_{name}"]),
                    helper.make_node("Where", [f"{name}_no8", f"idx{bc}", col_idx], [f"col_idx_{name}"]),
                ]
            )
            row_idx = f"row_idx_{name}"
            col_idx = f"col_idx_{name}"

    nodes.extend(
        [
            helper.make_node("Unsqueeze", [row_idx, "unsq_row_axis"], ["row_col"]),
            helper.make_node("Unsqueeze", [col_idx, "unsq_col_axis"], ["col_row"]),
            ...
            helper.make_node("GatherND", ["input", "gathernd_indices"], ["selected_onehot"], batch_dims=2),
    ...
    graph = helper.make_graph(nodes, "task011_dynamic_gathernd_block_graph", [x], [y], initializers)
